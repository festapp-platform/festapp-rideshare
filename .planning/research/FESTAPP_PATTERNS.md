...
- All tables: `UUID DEFAULT gen_random_uuid()` for consistency
- Avoids BIGINT/UUID mixing (auth.users forces UUID anyway)
- No enumeration risk, cleaner foreign keys
- Negligible performance difference at our scale
- **Divergence from Festapp:** Festapp uses BIGINT for non-user tables. We use UUID everywhere for simplicity.
