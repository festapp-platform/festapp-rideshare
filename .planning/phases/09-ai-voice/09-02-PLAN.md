...
2. **call_tool handler** — Switch on tool name:
   ...
   - `search_rides`: Accept origin/destination addresses. Use Supabase Edge Function `compute-route` via HTTP fetch (with service_role key from env) to geocode addresses into coordinates, then call `supabase.rpc('nearby_rides', { p_origin_lat, p_origin_lng, p_dest_lat, p_dest_lng, ... })` for proper geospatial search. If geocoding fails, fall back to text-based search: `supabase.from('rides').select('*').ilike('origin_address', '%${origin}%').ilike('destination_address', '%${dest}%')`.
