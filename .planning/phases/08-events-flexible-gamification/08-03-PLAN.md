...
must_haves:
  ...
  artifacts:
    - path: "supabase/migrations/00000000000035_gamification.sql"
      provides: "Achievement badges, user levels, route streaks, impact stats calculation"
...
Output: Gamification migration, shared types/constants/queries, impact dashboard page, badges and level components integrated into profiles and ride cards.
</objective>
...
<task type="auto">
  <name>Task 1: Gamification database migration and shared package</name>
  <files>
    supabase/migrations/00000000000035_gamification.sql
    packages/shared/src/types/database.ts
    packages/shared/src/constants/gamification.ts
  ...
  <action>
    ...
    **Shared package:**
    ...
    Database types: Add badge_definitions, user_achievements, route_streaks table types. Add get_user_impact, get_user_badges, get_route_streaks function signatures. Add derived types: BadgeDefinition, UserAchievement, RouteStreak, UserImpact (from get_user_impact return).
...
<task type="auto">
  ...
  <action>
    ...
    **Search results integration:**
    ...
    - Use the driver's completed_rides_count and rating_avg already available in nearby_rides results (driver_rating + need to add completed_rides_count to nearby_rides return if not present -- check and add to get_event_rides pattern)
    - If completed_rides_count is not in nearby_rides, add a `driver_level` computed field to the RideCard that uses the existing data (rating_count as proxy for activity, or just show level based on rating alone initially -- the nearby_rides RPC may need extension in migration 035 to include completed_rides_count)

    **Note on nearby_rides extension:** In migration 00000000000035, ALTER the nearby_rides function (CREATE OR REPLACE) to include `completed_rides_count` in the return type by joining with profiles. This allows RideCard to compute the user level.
...
2. Auto-award triggers fire on ride completion and review submission
3. get_user_impact RPC returns correct CO2/money/distance calculations
4. getUserLevel returns correct level based on rides+rating thresholds
5. Impact dashboard displays all stats, badges, and streaks
6. Badges visible on public profiles
7. User level shown on ride cards in search results
8. Share button copies impact text to clipboard
</verification>
