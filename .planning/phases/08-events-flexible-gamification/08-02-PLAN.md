...
<task type="auto">
  ...
  <action>
    ...
    **Shared package:**
    ...
    Database types: Add route_intents and route_intent_subscriptions table types. Add confirm_route_intent and cancel_route_intent function signatures. Add RouteIntent and RouteIntentSubscription derived types.
  ...
  <done>
    Route intents and subscriptions tables exist in migration. confirm_route_intent RPC creates a ride and notifies all subscribers via _notify(). Shared package has flexible ride constants, Zod schemas, Database types, and query builders all exported.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **ConfirmDate component (/routes/[id]/confirm-date.tsx):**
    ...
    - On submit: calls confirm_route_intent RPC
    - On success: toast "Date confirmed! Subscribers have been notified." and redirect to the new ride detail page
  ...
  <done>
    Driver can create a route intent at /routes/new. Passenger can browse intents at /routes and subscribe from /routes/[id]. Driver can confirm a date from intent detail page, which creates a ride and notifies subscribers. Confirmed intents link to the created ride for booking.
  </done>
</task>
...
2. confirm_route_intent RPC creates a ride and dispatches notifications to all subscribers via _notify()
...
6. Date confirmation form calls RPC and redirects to new ride
</verification>
